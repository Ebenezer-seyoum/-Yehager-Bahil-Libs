...
export const customerCreditRules = pgTable(
  "customer_credit_rules",
  {
    ...
    appliesTo: varchar("applies_to", { length: 32 }).default("other_orders").notNull(),
    status: varchar("status", { length: 32 }).default("active").notNull(),
    internalNote: text("internal_note"),
    ...timestamps,
  },
  (table) => [
    ...
    check("customer_credit_rules_applies_to_check", sql`${table.appliesTo} in ('all_orders', 'catalog_orders', 'custom_orders', 'other_orders')`),
...
export const profitCostSettings = pgTable(
  "profit_cost_settings",
  {
    ...
    taxPercent: numeric("tax_percent", { precision: 8, scale: 4 }).default("0").notNull(),
...
export const orders = pgTable(
  "orders",
  {
    ...
    creditUsedUsd: numeric("credit_used_usd", { precision: 12, scale: 2 }).default("0").notNull(),
    ...
    stripeAmountReceived: numeric("stripe_amount_received", { precision: 12, scale: 2 }),
    ...
    stripePaymentMethodFunding: text("stripe_payment_method_funding"),
    stripePaymentMethodCountry: text("stripe_payment_method_country"),
    ...
    stripeRefundAmount: numeric("stripe_refund_amount", { precision: 12, scale: 2 }),
    stripeReceiptMetadata: jsonb("stripe_receipt_metadata").$type<Record<string, unknown>>(),
    refundStatus: text("refund_status"),
    refundAmountEtb: numeric("refund_amount_etb", { precision: 14, scale: 2 }),
    refundProofUrl: text("refund_proof_url"),
    refundProofUploadedAt: timestamp("refund_proof_uploaded_at", { withTimezone: true }),
    refundCompletedAt: timestamp("refund_completed_at", { withTimezone: true }),
    refundCompletedBy: text("refund_completed_by"),
...
export const orderWorkstreams = pgTable(
  "order_workstreams",
  {
    ...
    deliveryStatus: varchar("delivery_status", { length: 40 }).default("not_started").notNull(),
    deliveryCarrier: text("delivery_carrier"),
    deliveryTrackingNumber: text("delivery_tracking_number"),
    deliveryStatusChangedBy: text("delivery_status_changed_by"),
    deliveryStatusChangedAt: timestamp("delivery_status_changed_at", { withTimezone: true }),
    deliveryTimeline: jsonb("delivery_timeline").$type<Array<{ status: string; note?: string; changedBy?: string; changedAt: string; trackingNumber?: string }>>().default([]),

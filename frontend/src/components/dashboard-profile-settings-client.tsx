...
function titleCase(v: unknown) {
  return value(v).split(/[._-]/g).filter(Boolean).map((p) => p[0]?.toUpperCase() + p.slice(1)).join(" ");
}
...
function initials(name?: string | null, email?: string | null) {
  return name?.split(" ").filter(Boolean).map((part) => part[0]).join("").slice(0, 2).toUpperCase() || email?.slice(0, 2).toUpperCase() || "U";
}
...
export function DashboardProfileSettingsClient({
  ...
  sessionUser: { name?: string | null; email?: string | null; assignedRoleName?: string | null; role?: string | null } | null;
  ...
  const assignedRole = isAdminProfile
    ? "Administrator"
    : profile?.assignedRoleName ?? sessionUser?.assignedRoleName ?? (profile?.roleStatus === "unassigned" ? "Access pending" : "Assigned employee");
  const profileFullName = [employeeProfile.firstName, employeeProfile.fatherName, employeeProfile.grandfatherName]
    .map((part) => String(part ?? "").trim())
    .filter(Boolean)
    .join(" ");
  const displayName = value(profileFullName || profile?.name || sessionUser?.name, "Account");
  const displayFirstFather = [employeeProfile.firstName, employeeProfile.fatherName]
    .map((part) => String(part ?? "").trim())
    .filter(Boolean)
    .join(" ");
  const avatarUrl = profile?.avatarUrl;
  const effectiveAvatarUrl = uploadedAvatarUrl ?? avatarPreviewUrl ?? avatarUrl ?? null;
  const editableSection = activeSection === "personal" || activeSection === "contact";
  const activeFormId = editableSection ? `profile-${activeSection}-form` : undefined;
  ...
  const sections = useMemo(
    () => {
      ...
      return [
        ...base,
        ...(variant === "employee" ? [{ id: "employment" as const, label: "Employment", icon: CalendarDays }] : []),
  ...
  async function submitProfile(section: SectionId, formData: FormData) {
    ...
    try {
      ...
      if (section === "contact") {
        ...
        body = {
          ...
          profile: {
            ...
            ...Object.fromEntries(
              Array.from(formData.entries()).map(([key, formValue]) => [key, String(formValue) || null]),
            ),
          },
        };
      }
      ...
      const savedFullName = [savedEmployeeProfile.firstName, savedEmployeeProfile.fatherName, savedEmployeeProfile.grandfatherName]
        .map((part) => String(part ?? "").trim())
        .filter(Boolean)
        .join(" ");
      const savedTopBarName = [savedEmployeeProfile.firstName, savedEmployeeProfile.fatherName]
        .map((part) => String(part ?? "").trim())
        .filter(Boolean)
        .join(" ");
      ...
      setNotice({ tone: "error", message: error instanceof Error ? error.message : "Could not save profile." });
    } finally {
      setBusy(false);
    }
  }
  ...
  async function handleAvatarFile(file?: File | null) {
    ...
    if (!allowedTypes.includes(file.type)) {
      setNotice({ tone: "error", message: "Please upload a JPG, PNG, or WEBP image." });
      return;
    }
    if (file.size > 5 * 1024 * 1024) {
      setNotice({ tone: "error", message: "Profile image must be 5MB or smaller." });
      return;
    }
    ...
    try {
      ...
      setNotice({ tone: "success", message: "Image ready. Click Update to save it." });
    } catch (error) {
    ...
    <div className="mx-auto w-full max-w-screen-2xl space-y-5">
      <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-orange-500">{variant}</p>
            <h1 className="mt-1 text-3xl font-black tracking-tight text-slate-950">Profile Settings</h1>
            <p className="mt-1 text-sm font-medium text-slate-500">Review profile details, update employee information, and manage password access.</p>
          </div>
          <button
            type="button"
      ...
      <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex min-w-0 items-center gap-5">
            {effectiveAvatarUrl ? (
              <img src={effectiveAvatarUrl} alt={displayName} className="h-24 w-24 rounded-3xl object-cover ring-4 ring-slate-100" />
            ) : (
            ...
            <div className="min-w-0">
              <p className="text-xs font-black uppercase tracking-[0.22em] text-orange-500">{variant}</p>
              <h1 className="mt-1 truncate text-3xl font-black tracking-tight text-slate-950">{displayName}</h1>
              <p className="mt-1 truncate text-sm font-bold text-slate-500">
                {displayFirstFather || value(profile?.email ?? sessionUser?.email)}
              </p>
            ...
            {editMode ? (
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                <div className="flex items-center gap-3">
                  {effectiveAvatarUrl ? (
                    <img src={effectiveAvatarUrl} alt={displayName} className="h-14 w-14 rounded-2xl object-cover ring-4 ring-white" />
                  ) : (
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs font-semibold text-slate-500">JPG, PNG, or WEBP up to 5MB</p>
                  </div>
                </div>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    selected ? "bg-slate-900 text-white shadow-md" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900",
                  )}
                >
                  <Icon className={cn("h-5 w-5", selected ? "text-emerald-300" : "text-slate-400")} />
        ...
        <main className="min-w-0">
          <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            <div className="mb-5 flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-black uppercase tracking-widest text-blue-700">{sections.find((s) => s.id === activeSection)?.label}</p>
                <h2 className="mt-1 text-xl font-black text-slate-950">Profile detail</h2>
              </div>
            </div>
            ...
            {activeSection === "personal" ? (
              editMode ? (
                isAdminProfile ? (
                  <form id="profile-personal-form" onSubmit={handleProfileSubmit("personal")} className="grid gap-4 md:grid-cols-2">
                    <Field label="Display name" name="name" defaultValue={profile?.name ?? sessionUser?.name} required />
                    <Detail label="Email Address" value={value(profile?.email ?? sessionUser?.email)} />
                  </form>
                ) : (
                  <form id="profile-personal-form" onSubmit={handleProfileSubmit("personal")} className="grid gap-4 md:grid-cols-3">
                    <Field label="First name" name="firstName" defaultValue={employeeProfile.firstName} required />
                    <Field label="Father name" name="fatherName" defaultValue={employeeProfile.fatherName} required />
                    <Field label="Grandfather name" name="grandfatherName" defaultValue={employeeProfile.grandfatherName} />
                    <Field label="Gender" name="gender" defaultValue={employeeProfile.gender} required />
                    <Field label="Date of birth" name="dateOfBirth" type="date" defaultValue={dateInput(employeeProfile.dateOfBirth)} />
                    <Field label="Marital status" name="maritalStatus" defaultValue={employeeProfile.maritalStatus} />
                  </form>
                )
              ) : (
                <div className="grid gap-3 md:grid-cols-2">
                  {isAdminProfile ? (
                    <>
                      ...
                      <Detail label="Email Address" value={value(profile?.email ?? sessionUser?.email)} />
                      <Detail label="Workspace Role" value={titleCase(profile?.role ?? sessionUser?.role ?? variant)} />
                      <Detail label="Account Status" value={titleCase(profile?.accountStatus ?? "active")} />
                    </>
                  ) : (
                    <>
                      <Detail label="Full Name" value={displayName} />
                      <Detail label="First Name" value={value(employeeProfile.firstName)} />
                      <Detail label="Father Name" value={value(employeeProfile.fatherName)} />
                      <Detail label="Grandfather Name" value={value(employeeProfile.grandfatherName)} />
                      <Detail label="Gender" value={titleCase(employeeProfile.gender)} />
                      <Detail label="Date of Birth" value={dateDisplay(employeeProfile.dateOfBirth)} />
                      <Detail label="Marital Status" value={titleCase(employeeProfile.maritalStatus)} />
                    </>
                  )}
                </div>
              )
            ...
            {activeSection === "contact" ? (
              editMode ? (
                <form id="profile-contact-form" onSubmit={handleProfileSubmit("contact")} className="grid gap-4 md:grid-cols-2">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  {!isAdminProfile ? (
                    <>
                      <Detail label="Country" value={value(employeeProfile.country)} />
                      <Detail label="City" value={value(employeeProfile.city)} />
                      <Detail label="Address" value={value(employeeProfile.address)} />
                    </>
                  ) : null}
                </div>
              )
            ...
            {activeSection === "employment" ? (
              <div className="grid gap-3 md:grid-cols-2">
                <Detail label="Employment Type" value={titleCase(employeeProfile.employmentType)} />
                <Detail label="Start Date" value={dateDisplay(employeeProfile.startDate)} />
                <Detail label="Invite Status" value={titleCase(employeeProfile.inviteStatus)} />
                <Detail label="Notes" value={value(employeeProfile.notes)} />
                <Detail label="Location" value={`${value(employeeProfile.city, "")}${employeeProfile.city && employeeProfile.country ? ", " : ""}${value(employeeProfile.country, "")}` || "Not provided"} />
                <Detail label="Address" value={value(employeeProfile.address)} />
              </div>
            ) : null}
            ...
            {activeSection === "access" ? (
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <Detail label="Role Status" value={titleCase(profile?.roleStatus ?? "assigned")} />
                <Detail label="Account Status" value={titleCase(profile?.accountStatus ?? "active")} />
                <Detail label="Created Date" value={dateDisplay(profile?.createdAt)} />
                <Detail label="Last Login" value={profile?.lastLoginAt ? new Date(profile.lastLoginAt).toLocaleString() : "Not provided"} />
              </div>
            ) : null}
            ...
            {activeSection === "password" ? (
              <form onSubmit={handlePasswordSubmit} className="grid gap-4 md:grid-cols-3">
                ...
                <button disabled={busy} className="inline-flex h-11 items-center gap-2 rounded-xl bg-blue-600 px-5 text-sm font-black text-white hover:bg-blue-700 disabled:opacity-60">

      ...
      {steps.map((label, index) => {
        ...
        return (
          <div key={label} className="flex items-center gap-2">
            ...
            >
              <span
                className={`grid h-4 w-4 place-items-center rounded-full border ${active ? "border-black" : "border-current"}`}
              >
    ...
    >
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <p
            className={`mt-2 text-[11px] font-bold ${required ? "text-primary" : "text-muted-foreground"}`}
          >
  ...
  return (
    <div>
      ...
      <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground sm:text-base">
        Bring your vision to life. Share your design references and preferences
        — our master tailors will craft your garment exactly as you envision it.
      </p>
...
export function UploadDesignWizard({
  ...
  eventId?: string;
  savedMeasurement?: SavedMeasurementSnapshot | null;
}) {
  const router = useRouter();
  const hasMeasurement = Boolean(savedMeasurement?.id);
  const [isMeasurementEditorOpen, setIsMeasurementEditorOpen] =
  ...
  const [hemStyle, setHemStyle] = useState(
    String(
      savedMeasurement?.hemStyle || savedMeasurement?.hem_style || "Straight",
    ),
  );
  const [pressingStyle, setPressingStyle] = useState(
    String(
      savedMeasurement?.pressingStyle ||
        savedMeasurement?.pressing_style ||
        "Creased",
    ),
  );
  ...
  const [detailImage, setDetailImage] = useState<UploadedFileState | null>(
    null,
  );
  ...
  const isKidsOutfit =
    outfitType.toLowerCase().includes("kid") ||
    outfitType.toLowerCase().includes("child");
  const childAgeNumber = Number(childAge);
  const isValidChildAge =
    childAge.trim() !== "" &&
    Number.isInteger(childAgeNumber) &&
    childAgeNumber >= 0 &&
    childAgeNumber <= 17;
  ...
  function canProceed() {
    ...
    if (step === 1)
      return Boolean(fabricType && embroideryStyle && colorPreference);
        ...
        body: JSON.stringify({
          ...
          measurementSnapshot: {
            ...
            ...(hasMeasurement && !isMeasurementEditorOpen
              ? savedMeasurement
              : measurementSnapshotFromStringValues(measurements, {
                  hemStyle,
                  pressingStyle,
                })),
    ...
    return (
      <div className="mx-auto grid min-h-[70vh] max-w-2xl place-items-center px-4 py-16 text-center">
        <div>
          ...
          <h1 className="mt-7 font-heading text-3xl font-bold">
            Design Request Submitted!
          </h1>
          <p className="mx-auto mt-5 max-w-xl text-lg leading-7 text-muted-foreground">
            Your custom design order{" "}
            <span className="font-black text-primary">{submittedNumber}</span>{" "}
            has been received. A confirmation email has been sent to you. Our
            team will be in touch within 1-2 business days.
          </p>
          <div className="mt-7 rounded-xl border border-primary/40 bg-primary/10 p-5 text-left text-sm leading-7 text-primary">
            ...
            <p>
              2. We may call or message you via WhatsApp for missing details.
            </p>
            <p>3. We send you a detailed price quote.</p>
            <p>
              4. Once you approve the quote and payment is confirmed, we begin
          ...
          <div className="mt-6 flex justify-center gap-3">
            {familyGroupId ? (
              <Link
                href={`/family-group/${familyGroupId}`}
                className="rounded-lg bg-primary px-5 py-2 text-sm font-bold text-black hover:bg-primary/90"
              >
            ...
            <Link
              href="/catalog"
              className="rounded-lg border border-border px-5 py-2 text-sm font-bold hover:bg-secondary"
            >
              Browse Catalog
            </Link>
            <Link
          ...
          <section className="rounded-2xl border border-border bg-card p-5 sm:p-6">
            <div className="mb-4">
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Upload photos of the style you have in mind. Front and back
                views are required.
              </p>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              <UploadSlot
                tag="Front View"
                icon="👕"
                label="Upload Front View Picture"
                required
                value={frontImage}
                busy={busy === "front"}
              ...
              <UploadSlot
                tag="Side View"
                icon="👔"
                label="Upload Side View Picture"
                value={sideImage}
                busy={busy === "side"}
                onUpload={(file) => handleUpload("side", file)}
              />
              <UploadSlot
                tag="Back View"
                icon="🔁"
                label="Upload Back View Picture"
                required
                value={backImage}
                busy={busy === "back"}
              ...
              <UploadSlot
                tag="Detail Close-Up"
                icon="🪡"
                label="Upload Embroidery / Pattern Close-up"
                value={detailImage}
                busy={busy === "detail"}
                onUpload={(file) => handleUpload("detail", file)}
              />
            </div>
          </section>
              ...
              {outfitTypes.map((option) => (
                <ChoiceButton
                  ...
                  onClick={() => {
                    ...
                    if (
                      !option.toLowerCase().includes("kid") &&
                      !option.toLowerCase().includes("child")
                    )
                      setChildAge("");
                  }}
                />
        ...
        <>
          <section className="rounded-2xl border border-border bg-card p-5 sm:p-6">
            ...
            <p className="mt-4 text-xs text-muted-foreground">
              Select the base fabric material for your garment.
            </p>
          ...
          <section className="rounded-2xl border border-border bg-card p-5 sm:p-6">
            <h2 className="font-heading text-xl font-bold">
              Embroidery & Tilet (ጥልፍ)
            </h2>
            <p className="mt-5 text-xs text-muted-foreground">
              Embroidery Type <span className="text-primary">*</span>
            </p>
            <div className="mt-2 grid gap-2 sm:grid-cols-2">
              {embroideryOptions.map((option) => (
                <ChoiceButton
                  key={option}
                  label={option}
                  selected={embroideryStyle === option}
                  onClick={() => setEmbroideryStyle(option)}
                />
              ))}
            </div>
            <p className="mt-4 text-xs text-muted-foreground">
              Primary Color Theme <span className="text-primary">*</span>
            </p>
            <div className="mt-2 grid gap-2 sm:grid-cols-2">
              {colorOptions.map((option) => (
                <ChoiceButton
                  key={option}
                  label={option}
                  selected={colorPreference === option}
                  onClick={() => setColorPreference(option)}
                />
              ))}
            </div>
          </section>
      ...
      {step === 2 ? (
        <section className="rounded-2xl border border-border bg-card p-5 sm:p-6">
          ...
          <div className="mt-7 flex flex-wrap items-center gap-3">
            ...
            <span className="ml-auto inline-flex items-center gap-2 text-xs font-bold text-primary">
              <BookOpen className="h-4 w-4" /> Measuring Guides
            </span>
            <button
              type="button"
              className="inline-flex items-center gap-2 rounded-lg bg-rose-600 px-4 py-2 text-xs font-black text-white"
            >
          ...
          <div className="mt-5 rounded-lg border border-primary/40 bg-primary/15 p-4 text-xs">
            <p className="font-black text-primary">
              Please measure in centimeters (cm) only.
            </p>
            <p className="mt-1 text-muted-foreground">
              Do not use inches — all values must be entered in CM to ensure a
              perfect fit.
            </p>
          </div>
          <p className="mt-5 text-xs text-muted-foreground">
          ...
          <div className="mt-2 grid gap-2 sm:grid-cols-2">
            <button
              ...
              className={`h-12 rounded-xl border text-sm font-bold ${gender === "female" ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground"}`}
            >
              👗 Women
            </button>
            <button
              ...
              className={`h-12 rounded-xl border text-sm font-bold ${gender === "male" ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground"}`}
            >
              👔 Men
            </button>
          </div>
          {hasMeasurement && !isMeasurementEditorOpen ? (
            <div className="mt-5 rounded-2xl border border-white/10 bg-[#1a1a1a] p-5 shadow-[0_10px_30px_rgba(0,0,0,0.35)]">
              ...
              <div className="mt-4 space-y-5">
                {measurementDisplayGroups(savedMeasurement ?? {})
                  .filter((group) => group.title !== "Profile")
                  .map((group) => (
                    <div key={group.title}>
                      <h3 className="mb-2 text-[11px] font-black uppercase tracking-[0.2em] text-primary">
                        {group.title}
                      </h3>
            ...
            <div className="mt-5 rounded-xl border border-primary/50 bg-primary/10 p-4">
              <div className="flex items-center justify-between gap-4">
                ...
                <button
                  type="button"
                  onClick={() => setMeasurementsSaved(false)}
            ...
            <>
              <div className="mt-5 rounded-xl border border-border p-4">
                ...
                <p className="mt-1 border-b border-primary/30 pb-3 text-xs text-muted-foreground">
                  Measurements for the top garment — shirt, suit coat, or blazer
                </p>
              ...
              <div
                className={`mt-5 overflow-hidden rounded-xl border border-border ${isPantsOpen ? "pb-4" : ""}`}
              >
                <button
                  type="button"
                ...
                >
                  ...
                  <ChevronDown
                    className={`h-4 w-4 transition-transform ${isPantsOpen ? "rotate-180" : ""}`}
                  />
                </button>
                {isPantsOpen && (
                        ...
                        <div className="mt-2 grid gap-2">
                          {HEM_STYLE_OPTIONS.map((option) => (
                            <ChoiceButton
                              key={option.value}
                              label={option.title}
                              selected={hemStyle === option.value}
                              onClick={() => {
                                setMeasurementsSaved(false);
                                setHemStyle(option.value);
                              }}
                            />
                          ))}
                        </div>
                        ...
                        <div className="mt-2 grid gap-2">
                          {PRESSING_STYLE_OPTIONS.map((option) => (
                            <ChoiceButton
                              key={option.value}
                              label={option.title}
                              selected={pressingStyle === option.value}
                              onClick={() => {
                                setMeasurementsSaved(false);
                                setPressingStyle(option.value);
                              }}
                            />
                          ))}
                        </div>
                ...
                <div className="flex gap-3 border-b border-primary/30 bg-primary/5 p-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs leading-5 text-muted-foreground">
                      Is there anything specific you&apos;d like our tailors to
                      know? Share any fit preferences, design details, or
                ...
                <div className="p-4 text-sm">
                  {measurementNoteOpen ? (
                    ...
                    <p className="italic text-muted-foreground">
                      {measurementNote || "No note added."}
                    </p>
                  )}
                  <button
                    type="button"
                    onClick={() => setMeasurementNoteOpen((open) => !open)}
                    className="mt-4 text-xs font-black text-primary"
                  >
                    ✎{" "}
              ...
              <button
                type="button"
                onClick={() => {
                  const requiredFields = TOP_MEASUREMENT_FIELDS.filter(
      ...
      {step === 3 ? (
        <>
          <section className="rounded-2xl border border-border bg-card p-5 sm:p-6">
            ...
            <p className="mt-4 text-xs text-muted-foreground">
              We&apos;ll send your quote and production updates via WhatsApp.
            </p>
            <label className="mt-4 block text-xs text-muted-foreground">
              ...
              <div className="mt-1 flex gap-2">
                ...
                <input
                  ...
                  className="h-11 min-w-0 flex-1 rounded-lg border border-primary bg-background px-3 text-sm text-foreground outline-none"
                />
              </div>
            </label>
          </section>
          ...
          <section className="rounded-2xl border border-border bg-card p-5 sm:p-6">
            <h2 className="font-heading text-2xl font-bold">
              Special Instructions & Tailor Remarks
            </h2>
            <p className="mt-4 text-xs text-muted-foreground">
              Optional but highly recommended — the more detail you provide, the
              better the outcome.
            </p>
            <textarea
          ...
          <section className="rounded-2xl border border-primary/40 bg-primary/10 p-5 text-sm">
            ...
            <div className="mt-3 grid grid-cols-[130px_1fr] gap-y-1 text-xs">
              <span className="font-bold text-foreground">Outfit:</span>
              <span className="text-muted-foreground">{outfitType}</span>
              {isKidsOutfit ? (
                <>
                  <span className="font-bold text-foreground">Child Age:</span>
                  <span className="text-muted-foreground">
                    {childAge || "Not provided"}
                  </span>
                </>
              ) : null}
              <span className="font-bold text-foreground">Fabric:</span>
              <span className="text-muted-foreground">{fabricType}</span>
              <span className="font-bold text-foreground">Embroidery:</span>
              <span className="text-muted-foreground">{embroideryStyle}</span>
              <span className="font-bold text-foreground">Color Theme:</span>
              <span className="text-muted-foreground">{colorPreference}</span>
              <span className="font-bold text-foreground">Sizing:</span>
              <span className="text-muted-foreground">Custom Measurements</span>
              <span className="font-bold text-foreground">Images:</span>
              <span className="text-muted-foreground">
            ...
            <div className="mt-4 border-t border-primary/20 pt-3 text-xs text-muted-foreground">
              <p>
                Pricing is not fixed — our team will send a detailed quote
                within 1-2 business days after reviewing your design.
              </p>

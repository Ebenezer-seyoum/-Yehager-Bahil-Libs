...
function RecordsTable({ tab, rows }: { tab: TableTab; rows: Row[] }) {
  return (
                ...
                <td className="px-5 py-4">
                  <DashboardTableActions>
                    <DashboardActionButton
                      action="view"
                      href={`/admin/finance/coupons-discounts/${tab}/${row.id}`}
                      aria-label={`View ${tab === "discounts" ? row.name ?? "discount" : row.code ?? "coupon"} details`}
                    />
                  </DashboardTableActions>
                </td>
              </tr>

...
export default async function AdminOrderDetailPage({ params, searchParams }) {
  ...
  const scope = query?.scope === "catalog" || query?.scope === "custom" ? query.scope : null;

  let order = null;
  try {
    const response = await apiRequest(`/api/v1/orders/${id}${scope ? `?scope=${scope}` : ""}`);
  ...
  return <OrderDetailPage initialOrder={order} backUrl={backUrl} scope={scope} />;
}

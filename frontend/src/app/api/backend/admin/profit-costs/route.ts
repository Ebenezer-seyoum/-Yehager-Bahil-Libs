...
export async function GET() {
  try {
    return NextResponse.json(await apiRequest("/api/v1/admin/profit-costs"));
  } catch (error) {
    const message = error instanceof Error ? error.message : "Profit and cost data load failed";
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    return NextResponse.json(await apiRequest("/api/v1/admin/profit-costs/settings", { method: "PATCH", body }));
  } catch (error) {
    const message = error instanceof Error ? error.message : "Profit and cost settings update failed";
    return NextResponse.json({ error: readableMessage(message) }, { status: statusFromMessage(message) });
  }
}

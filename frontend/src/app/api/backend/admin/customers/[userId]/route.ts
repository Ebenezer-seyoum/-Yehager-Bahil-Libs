...
export async function GET(_: Request, { params }: { params: Promise<{ userId: string }> }) {
  ...
  try {
    const data = await apiRequest(`/api/v1/admin/customers/${userId}`);
    return NextResponse.json(data, { status: 200 });
  } catch (error) {
    return apiError(error, "Customer fetch failed");
  }
}
...
export async function PATCH(request: Request, { params }: { params: Promise<{ userId: string }> }) {
  ...
  try {
    ...
    return apiError(error, "Customer update failed");
  }
}
...
export async function DELETE(request: Request, { params }: { params: Promise<{ userId: string }> }) {
  ...
  try {
    const data = await apiRequest(`/api/v1/admin/customers/${userId}${force ? "?force=true" : ""}`, { method: "DELETE" });
    return NextResponse.json(data, { status: 200 });
  } catch (error) {
    return apiError(error, "Customer delete failed");
  }
}

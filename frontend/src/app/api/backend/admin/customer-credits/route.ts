...
export async function GET() {
  try {
    return NextResponse.json(await apiRequest("/api/v1/admin/customer-credits"));
  } catch (error) {
    const message = error instanceof Error ? error.message : "Customer credits load failed";
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    return NextResponse.json(await apiRequest("/api/v1/admin/customer-credits/rule", { method: "PATCH", body }));
  } catch (error) {
    const message = error instanceof Error ? error.message : "Credit rule update failed";
    return NextResponse.json({ error: readableMessage(message) }, { status: statusFromMessage(message) });
  }
}

...
export default async function AdminCustomerDetailPage({ params, searchParams }) {
  ...
  try {
    ...
    customer = Array.isArray(response?.data) ? response.data.find((item) => item.id === id && item.role === "customer") ?? null : null;
  } catch {
    customer = null;
  }
  ...
  async function updateProfile(formData) {
    ...
    try {
      ...
      redirect(`/admin/customers/${id}?updated=profile`);
    } catch {
      redirect(`/admin/customers/${id}?error=profile`);
    }
  }
  ...
  async function suspendCustomer() {
    ...
    try {
      ...
      revalidatePath(`/admin/customers/${id}`);
      redirect(`/admin/customers/${id}?updated=status`);
    } catch {
  ...
  async function resetPassword(formData) {
    ...
    try {
      ...
      revalidatePath(`/admin/customers/${id}`);
      redirect(`/admin/customers/${id}?updated=password`);
    } catch {
      redirect(`/admin/customers/${id}?error=password`);
    }
  }
    ...
    <div className="mx-auto w-full max-w-7xl space-y-6">
      {query.updated ? (
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-medium text-emerald-800 shadow-sm">
          Success — the customer action was completed.
        </div>
      ) : null}
      {query.error ? (
